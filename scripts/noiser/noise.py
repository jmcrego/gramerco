...
def output(toks, tags, args):
    ...
    if args.output_pair:
        ...
        print("{}".format(
            ' '.join([toks[i] + separ + tags[i] for i in range(len(toks))])))
...
def do_spacy(nlp, l, toks):
    ...
    for token in tokens:
        ...
        if toks.count(txt) != 1:
            continue
        pos = str(token.pos_)
        if pos != 'NOUN' and pos != 'ADJ' and pos != 'VERB' and pos != 'AUX':
            continue
...
def misspell(word):
    ...
    if r < 1.0 / 4:  # add extra character in position k [0, len(word)]
        random.shuffle(chars)
        k = random.randint(0, len(word))
        word.insert(k, chars[0])
    elif r < 2.0 / 4:  # remove character in position k [0, len(word)-1]
        k = random.randint(0, len(word) - 1)
        l = word.pop(k)
    # swap characters in positions k and k+1, k in [0, len(word)-2]
    elif r < 3.0 / 4:
        k = random.randint(0, len(word) - 2)
        # if word[k] == word[k=1] the returned word is the same!
...
def do_replace_inflection(toks, tags, vocab, wrd2wrds_same_lemma, seen, args):
    # replace a random word by any in wrd2wrds[word] if word in vocab, tag the
    # previous as REPLACE:INFLECTION_word
...
def do_replace_homophone(toks, tags, vocab, wrd2wrds_homophones, seen, args):
    # replace a random word by any in wrd2wrds[word] if word in vocab, tag the
    # previous as REPLACE:HOMOPHONE_word
    idxs = list(range(len(toks)))
    random.shuffle(idxs)
    for idx in idxs:
        if tags[idx] != keep:
...
def do_append(toks, tags, vocab, seen, args):
...
def do_inflect(toks, tags, txt2inf, lempos2wrds, seen, args):
    # replace word i if in vocab by another with different inflection, tag it
    # with INFLECT:inflection
    idxs = list(range(len(toks) - 1))
    random.shuffle(idxs)
    for idx in idxs:
        if tags[idx] != keep:
        ...
        if new_txt == '':
            continue
        curr_inflection = ';'.join(
            curr_inflection.split(';')[
                1:])  # discard lemma
...
def do_hyphen_split(toks, tags, vocab, seen, args):
    # take two consecutive words and join them with an hyphen, tag it as
    # HYPHEN_SPLIT
    idxs = list(range(len(toks) - 1))
    random.shuffle(idxs)
    for idx in idxs:
...
def do_hyphen_merge(toks, tags, vocab, seen, args):
    # take a word with an hyphen and split it into two, tag the first as
    # HYPHEN_MERGE
    idxs = list(range(len(toks)))
    random.shuffle(idxs)
    for idx in idxs:
...
def noise_line(
    ...
    while n_attempts < args.wmax * 2 and n_changes < args.wmax:
        ...
        if choice == 'replace:inflection':
            n_changes += do_replace_inflection(toks,
                                               tags,
                                               vocab,
                                               wrd2wrds_same_lemma,
                                               seen,
            ...
            n_changes += do_replace_homophone(toks,
                                              tags,
                                              vocab,
                                              wrd2wrds_homophones,
                                              seen,
            ...
            n_changes += do_append(toks, tags, vocab, seen, args)
        elif choice == 'inflect':
            n_changes += do_inflect(toks, tags, txt2inf,
                                    lempos2wrds, seen, args)
        elif choice == 'split':
...
def read_lexicon(f, vocab):
    ...
    with open(f, 'r') as fd:
        for l in fd:
            ...
            if len(toks) < 3:
                continue
            wrd, pho, lem, pos, gender, nbre, conj = toks[0], toks[
                1], toks[2], toks[3], toks[4], toks[5], toks[10]
            if wrd not in vocab:
                continue
            ...
            lempos2wrdsinf[lem + separ +
                           pos].add((wrd, pos, gender, nbre, conj))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--vocab',
        help='File with words to consider (required)',
        required=True)
    parser.add_argument(
        '--lexicon',
        help='Lexique383.tsv lexicon file (required)',
        required=True)
    parser.add_argument(
        '--wmax',
        type=int,
        default=5,
        help='Max number of words noised per sentence (5)')
    parser.add_argument(
        '--p_clean',
        type=float,
        default=0.2,
        help='Prob of printing an unnoised sentence (0.2)')

    parser.add_argument(
        '--w_replace_inflection',
        type=float,
        default=5,
        help='Weight of replacing a word inflection (5)')
    parser.add_argument(
        '--w_replace_homophone',
        type=float,
        default=5,
        help='Weight of replacing a word with an homophone (5)')
    parser.add_argument(
        '--w_replace_spell',
        type=float,
        default=5,
        help='Weight of replacing a misspelled word (5)')
    parser.add_argument('--w_split', type=float, default=5,
                        help='Weight of splitting one word (5)')
    parser.add_argument(
        '--w_append',
        type=float,
        default=5,
        help='Weight of appending one word (5)')

    parser.add_argument(
        '--w_inflect',
        type=float,
        default=5,
        help='Weight of inflecting a word (5)')
    parser.add_argument(
        '--w_delete',
        type=float,
        default=1,
        help='Weight of deleting a random/copied word (1)')
    parser.add_argument('--w_swap', type=float, default=1,
                        help='Weight of swapping two words (1)')
    parser.add_argument('--w_merge', type=float, default=1,
                        help='Weight of merging two words (1)')
    parser.add_argument(
        '--w_hyph_merge',
        type=float,
        default=1,
        help='Weight of merging two words with an hyphen (1)')
    parser.add_argument(
        '--w_hyph_split',
        type=float,
        default=1,
        help='Weight of splitting one word with an hyphen (1)')
    parser.add_argument(
        '--w_case_first',
        type=float,
        default=1,
        help='Weight of changing the first char case (1)')
    parser.add_argument(
        '--w_case_upper',
        type=float,
        default=1,
        help='Weight of uppercasing a word (1)')
    parser.add_argument(
        '--w_case_lower',
        type=float,
        default=1,
        help='Weight of lowercasing a word (1)')

    parser.add_argument(
        '--only_vocab',
        action='store_true',
        help='Weights of noises not generating words are set to 0')
    parser.add_argument(
        '--output_pair',
        action='store_true',
        help='Output tab-separated strings of words/tags')
    parser.add_argument('--seed', type=int, default=0,
                        help='Seed for randomness (0)')
    ...
    args.noises = [
        ...
        'case:lower']
    args.weights = [
        args.w_replace_inflection,
        args.w_replace_homophone,
        args.w_replace_spell,
        args.w_split,
        args.w_append,
        args.w_inflect,
        args.w_delete,
    ...
    wrd2wrds_same_lemma, wrd2wrds_homophones, lempos2wrds = read_lexicon(
        args.lexicon, vocab)
    tic = time.time()
    nsents = 0
    ntokens = 0
    ...
    toc = time.time()
    sys.stderr.write(
        'Processed {} input sentences in {:.2f} seconds\n'.format(
            nsents, toc - tic))
